...
exports.Progress = function(){
  ...
  function printMsg() {
    switch (counter) {
      case 0:
        console.log('Use `clever --help` from command line for all CLI options');
        break;
      case 1:
        console.log('Be sure to checkout all the docs on http://clever.imperodesign.com');
        break;
      case 2:
...
exports.loadPackageJson = function(path, callback) {
  fs.readFile(path, function(err, data) {
    ...
    try {
      ...
      pkg.cleverVersion = pkg.clever || pkg.version;
      callback(null, pkg);
    } catch (err) {
      return callback(err);
    }
